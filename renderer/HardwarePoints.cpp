...
void g3_ProjectPoint(g3Point *p) {
  ...
  if (sStereoFrustumValid) {
    ...
    if (clip_w != 0.0f) {
      ...
      static int stereo_project_log_count = 0;
      if (stereo_project_log_count < 256) {
        LOG_INFO.printf(
            "VR-MENU-TRACE: g3_ProjectPoint stereo p=(%f,%f,%f) proj00=%f proj11=%f proj20=%f proj21=%f clip=(%f,%f,%f) "
            "ndc=(%f,%f) screen=(%f,%f) w2=(%f,%f)",
            p->p3_x, p->p3_y, p->p3_z, gTransformProjection[0][0], gTransformProjection[1][1],
            gTransformProjection[2][0], gTransformProjection[2][1], clip_x, clip_y, clip_w, ndc_x, ndc_y, p->p3_sx,
            p->p3_sy, Window_w2, Window_h2);
        stereo_project_log_count++;
      }
    }
  } else {
